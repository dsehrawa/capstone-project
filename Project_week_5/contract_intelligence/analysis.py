...
INTERNAL_RISK_RULES = [
    {"risk_name": "Termination / early exit risk", "keywords": ["terminate", "termination", "expiration", "end date", "renewal"], "category": "Contractual", "default_impact": "High", "explanation": "Termination clauses can create revenue disruption, service interruption, or abrupt obligation changes."},
    {"risk_name": "Payment / fee risk", "keywords": ["payment", "fee", "charge", "invoice", "tax", "late payment"], "category": "Financial", "default_impact": "High", "explanation": "Payment timing, fee disputes, or tax allocation can directly affect cash flow and commercial viability."},
    {"risk_name": "Confidentiality / data handling risk", "keywords": ["confidential", "proprietary", "non-disclosure", "nondisclosure", "secret"], "category": "Information Security", "default_impact": "High", "explanation": "Disclosure of confidential information can trigger legal exposure, reputational damage, and commercial harm."},
    {"risk_name": "Liability / indemnity exposure", "keywords": ["indemnify", "indemnification", "hold harmless", "liabilities", "damages", "limitation of liability"], "category": "Legal", "default_impact": "High", "explanation": "Broad indemnity or liability language can create significant financial and legal exposure."},
    {"risk_name": "Regulatory / compliance risk", "keywords": ["law", "jurisdiction", "arbitration", "court", "compliance", "applicable law"], "category": "Compliance", "default_impact": "Medium", "explanation": "Jurisdiction, dispute resolution, and compliance obligations can increase cost and enforcement complexity."},
    {"risk_name": "Intellectual property ownership risk", "keywords": ["intellectual property", "patent", "trademark", "copyright", "license", "ownership", "rights"], "category": "IP", "default_impact": "High", "explanation": "Ambiguous IP ownership or license scope can affect product rights, reuse, and future commercialization."},
    {"risk_name": "Force majeure / service disruption risk", "keywords": ["force majeure", "act of god", "delay", "failure to perform", "event beyond control"], "category": "Operational", "default_impact": "High", "explanation": "Disruption clauses indicate exposure to uncontrollable events that may delay or prevent performance."},
]

OBLIGATION_PATTERNS = [r"\bshall\b", r"\bmust\b", r"\bagrees? to\b", r"\bis required to\b", r"\bwill\b", r"\bundertakes? to\b"]
EXTERNAL_RISK_CONTEXT = {"weather_area": "NY", "cyber_vendor_keywords": ["microsoft", "apache", "oracle", "linux", "openssl"], "cyber_product_keywords": ["exchange", "windows", "http server", "mysql", "vmware"]}
COUNTRY_NAME_TO_CODE = {"united states": "USA", "u.s.": "USA", "u.s.a.": "USA", "usa": "USA", "india": "IND", "united kingdom": "GBR", "uk": "GBR", "england": "GBR", "canada": "CAN", "australia": "AUS", "germany": "DEU", "france": "FRA", "singapore": "SGP", "japan": "JPN", "china": "CHN", "uae": "ARE", "united arab emirates": "ARE"}
STATE_TO_COUNTRY_CODE = {"new york": "USA", "california": "USA", "texas": "USA", "florida": "USA", "delaware": "USA", "massachusetts": "USA", "illinois": "USA", "washington": "USA", "new jersey": "USA"}
...
def classify_clause(clause_text: str) -> str:
    ...
    for category, keywords in CLAUSE_CATEGORIES.items():
        if category == "General Provisions":
            continue
        current_keyword_matches = sum(1 for keyword in keywords if keyword in clause_text_lower)
...
def extract_obligations(combined_legal_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in combined_legal_data:
        ...
        if any(re.search(pattern, clause_text_lower) for pattern in OBLIGATION_PATTERNS):
            ...
            obligations.append({"party": detect_party_name(clause_text, entities), "obligation_type": detect_obligation_type(clause_text_lower), "category": item.get("category", "General Provisions"), "clause_text": clause_text})
    return obligations
...
def hybrid_risk_classifier(clause_text: str, category_hint: str | None = None) -> dict[str, Any]:
    ...
    for category_name, config in RISK_TAXONOMY.items():
        keyword_hits = sum(1 for keyword in config["keywords"] if keyword in clause_text_lower)
        if category_hint and category_hint.lower() in category_name.lower():
            keyword_hits += 1
        rule_scores[category_name] = keyword_hits
    top_category, top_score = max(rule_scores.items(), key=lambda item: item[1])
    if top_score == 0:
    ...
    return {"predicted_risk_category": top_category, "likelihood": likelihood, "impact": impact, "overall_score": SCORE_MAP[likelihood] * SCORE_MAP[impact], "rule_score": top_score, "llm_review_required": top_score <= 1}


def safe_get_json(url: str, timeout: int = 30) -> Any:
    response = requests.get(url, timeout=timeout, headers={"User-Agent": "contract-risk-intelligence-app"})
...
def fetch_noaa_weather_risk(area: str) -> dict[str, Any]:
    ...
    severe_terms = ["Tornado", "Hurricane", "Flood", "Storm", "Blizzard", "Fire", "Heat", "Ice"]
    severe_alerts = []
    for feature in payload.get("features", []):
        props = feature.get("properties", {})
        event = props.get("event", "")
        if any(term.lower() in event.lower() for term in severe_terms):
            severe_alerts.append({"event": event, "severity": props.get("severity"), "headline": props.get("headline")})
    likelihood = level_from_count(len(severe_alerts), medium_threshold=1, high_threshold=3)
...
def fetch_cisa_kev_risk(vendor_keywords: list[str], product_keywords: list[str]) -> dict[str, Any]:
    ...
    for item in payload.get("vulnerabilities", []):
        ...
        if any(keyword.lower() in searchable for keyword in vendor_keywords + product_keywords):
            matched.append({"cveID": item.get("cveID"), "vendorProject": item.get("vendorProject"), "product": item.get("product"), "dateAdded": item.get("dateAdded")})
    likelihood = level_from_count(len(matched), medium_threshold=1, high_threshold=10)
...
def infer_country_codes_from_contract(combined_legal_data: list[dict[str, Any]], document_metadata: dict[str, Any], fallback_codes: list[str] | None = None, max_countries: int = 3) -> list[str]:
    ...
    inferred_codes = [country_code for country_code, _ in candidate_counter.most_common(max_countries)]
    ...
    for url in candidate_urls:
        try:
            ...
            if not isinstance(payload, list) or len(payload) < 2:
                continue
            valid_points = [row for row in payload[1] if row.get("value") is not None]
            if valid_points:
            ...
            last_error = exc
    raise ValueError(f"World Bank lookup failed for {country_code} / {indicator_code}: {last_error}")
...
def fetch_world_bank_macro_risk(country_code: str) -> dict[str, Any]:
    inflation = fetch_world_bank_indicator_latest(country_code, "FP.CPI.TOTL.ZG")
    gdp_growth = fetch_world_bank_indicator_latest(country_code, "NY.GDP.MKTP.KD.ZG")
    likelihood, impact, signal_points = score_macro_risk_from_indicators(inflation.get("value"), gdp_growth.get("value"))
...
def collect_internal_risk_evidence(risk_rule: dict[str, Any], combined_legal_data: list[dict[str, Any]]) -> dict[str, Any]:
    matched_clauses = [item.get("clause_text", "") for item in combined_legal_data if any(keyword in item.get("clause_text", "").lower() for keyword in risk_rule["keywords"])]
...
def run_external_risk_analysis(combined_legal_data: list[dict[str, Any]], document_metadata: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    try:
        live_external_risks.append(fetch_noaa_weather_risk(area=EXTERNAL_RISK_CONTEXT["weather_area"]))
    except Exception as exc:
        live_external_errors.append({"risk_name": "Weather / natural disaster risk", "error": str(exc)})
    try:
        live_external_risks.append(fetch_cisa_kev_risk(EXTERNAL_RISK_CONTEXT["cyber_vendor_keywords"], EXTERNAL_RISK_CONTEXT["cyber_product_keywords"]))
    except Exception as exc:
        live_external_errors.append({"risk_name": "Cybersecurity risk", "error": str(exc)})
    for country_code in infer_country_codes_from_contract(combined_legal_data, document_metadata):
        try:
            live_external_risks.append(fetch_world_bank_macro_risk(country_code))
        except Exception as exc:
            live_external_errors.append({"country_code": country_code, "error": str(exc)})
...
def analyze_contract(pdf_path: str | Path) -> AnalysisArtifacts:
    ...
    for clause in clauses:
        combined_legal_data.append({"clause_text": clause, "category": classify_clause(clause), "entities": extract_entities(nlp, clause)})
    document_metadata = build_document_metadata(combined_legal_data, cleaned_text)
    obligations = extract_obligations(combined_legal_data)
    hybrid_clause_risks = []
    for item in combined_legal_data:
        hybrid_clause_risks.append({"clause_text": item["clause_text"], "original_clause_category": item["category"], **hybrid_risk_classifier(item["clause_text"], category_hint=item["category"])})
    internal_risks = [collect_internal_risk_evidence(rule, combined_legal_data) for rule in INTERNAL_RISK_RULES]
    live_external_risks, live_external_errors = run_external_risk_analysis(combined_legal_data, document_metadata)
    clauses_df = pd.DataFrame(hybrid_clause_risks)
    obligations_df = pd.DataFrame(obligations)
    risk_category_summary_df = clauses_df.groupby("predicted_risk_category", as_index=False).agg(clause_count=("clause_text", "count"), avg_score=("overall_score", "mean"), llm_review_count=("llm_review_required", "sum")).sort_values(by=["avg_score", "clause_count"], ascending=[False, False])
    obligation_summary_df = obligations_df.groupby(["party", "obligation_type"], as_index=False).size().rename(columns={"size": "obligation_count"}).sort_values(by="obligation_count", ascending=False) if not obligations_df.empty else pd.DataFrame(columns=["party", "obligation_type", "obligation_count"])
    heatmap_df = clauses_df.groupby(["likelihood", "impact"]).size().unstack(fill_value=0).reindex(index=["Low", "Medium", "High"], columns=["Low", "Medium", "High"], fill_value=0)
    all_live_external_df = pd.DataFrame(live_external_risks)
    combined_priority_risks_df = pd.concat([clauses_df[["predicted_risk_category", "likelihood", "impact", "overall_score"]].rename(columns={"predicted_risk_category": "risk_name"}), all_live_external_df[["risk_name", "likelihood", "impact", "overall_score"]] if not all_live_external_df.empty else pd.DataFrame(columns=["risk_name", "likelihood", "impact", "overall_score"])], ignore_index=True).sort_values(by="overall_score", ascending=False)
    structured_output = {"metadata": document_metadata, "full_text": cleaned_text, "clauses": combined_legal_data, "risk_obligation_intelligence": {"taxonomy": RISK_TAXONOMY, "internal_risks": internal_risks, "hybrid_clause_risks": hybrid_clause_risks, "obligations": obligations, "risk_category_summary": risk_category_summary_df.to_dict(orient="records"), "obligation_summary": obligation_summary_df.to_dict(orient="records"), "live_external_risks": live_external_risks, "live_external_errors": live_external_errors}}
