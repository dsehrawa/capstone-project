...
def judge_with_llm(question: str, expected: str, actual: str) -> dict:
    ...
    raw = ollama_chat("mistral", prompt, timeout=120)
...
def run_evaluation(verbose: bool = False) -> dict:
    ...
    # Build artifacts for each contract
    ...
    for item in golden_set:
        ...
        if contract not in retriever_map:
            ...
            results.append({**item, "actual_answer": "SKIPPED",
                           "score": 0, "reason": "Contract not loaded"})
            continue

        print(f"  Q{qid}: {question[:80]}...")
        start_t = time.time()
        result = answer_question(
            question, retriever_map[contract], risk_summary_map[contract])
    ...
    # Summary
    answered = [r for r in results if r["score"] > 0]
    avg_score = total_score / len(golden_set) if golden_set else 0
    score_dist = {
        s: sum(1 for r in results if r["score"] == s) for s in range(6)}
